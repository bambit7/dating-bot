...
def save_or_update_girl(telegram_id, name, age, region, bio, contact, photo):
    ...
    if old:
        cur.execute("""
            UPDATE girls SET name=?, age=?, region=?, bio=?, contact=?, photo=?, approved=1
            WHERE telegram_id=?
...
def get_girls_by_region(region):
    ...
    cur = conn.cursor()
    cur.execute("""
        SELECT id, name, age, region, bio, photo FROM girls
...
def get_all_girls():
    conn = db()
    cur = conn.cursor()
    cur.execute("SELECT id, name, region, bio, contact FROM girls ORDER BY id DESC")
    data = cur.fetchall()
    conn.close()
    return data
...
def delete_girl_by_id(girl_id):
    conn = db()
    cur = conn.cursor()
    cur.execute("DELETE FROM girls WHERE id=?", (girl_id,))
...
async def girls_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for girl_id, name, region, bio, contact in girls:
        text += (
            f"🆔 ID: <b>{girl_id}</b>\n"
            f"👤 Ism: {name}\n"
            f"📍 Viloyat: {region}\n"
...
async def delete_girl(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("O‘chirish uchun ID yozing.\nMasalan: /delete 5")
        return
    ...
    if deleted:
        await update.message.reply_text(f"✅ {girl_id}-ID profil o‘chirildi.")
    else:
...
async def region(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data["gender"] == "👧 Qiz":
        if get_existing_profile(update.effective_user.id):
            await update.message.reply_text(
                "ℹ️ Sizda oldin profil bor.\nYangi ma’lumot yuborsangiz, profilingiz yangilanadi."
            )
...
async def bio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(update.message.text) < 5:
        await update.message.reply_text("❌ Juda qisqa yozildi. Qayta yozing.")
        return BIO
...
async def contact(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("✅ Profilingiz adminga yuborildi.", reply_markup=ReplyKeyboardRemove())
    return ConversationHandler.END
...
async def buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("approve_"):
        ...
        await query.edit_message_caption(caption="✅ Profil tasdiqlandi.")
        ...
        if index >= len(girls):
            await query.message.reply_text("😕 Boshqa profil yo‘q.")
            return
